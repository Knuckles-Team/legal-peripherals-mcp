...
| `OPENCORPORATES_BASE_URL` | optional | Override for a self-hosted/proxy registry (default `[configured-endpoint]`) |
...
## Gotchas
- No `OPENCORPORATES_API_TOKEN` ⇒ the tool truthfully reports "not configured" and
  returns no record, so there is nothing to ingest.
