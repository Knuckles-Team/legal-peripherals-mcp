...
def _validate_statute_inputs(
    ...
    if len(topic_norm) > 200:
        raise StatuteLookupError(f"Topic too long ({len(topic_norm)} chars). Max 200.")
...
async def handle_statute_rules(
    ...
    try:
        state_upper, type_norm, topic_norm = _validate_statute_inputs(
